...
public class PsicologosController : Controller
{
    ...
    public PsicologosController(UserManager<ApplicationUser> userManager)
    {
    ...
    [HttpPost]
    public async Task<IActionResult> Create(string nombre, string apellidos, string email, string turno)
    {
        var psicologo = new ApplicationUser
        {
            UserName = email,
            Email = email,
            Nombre = nombre,
        ...
        var result = await _userManager.CreateAsync(psicologo, "Psico123*");

        if (result.Succeeded)
        {
            await _userManager.AddToRoleAsync(psicologo, "Psicologo");
            return RedirectToAction("Index");
        }

        ViewBag.Error = "Error al crear psicólogo";
        return View();
    }

    public async Task<IActionResult> Index()
    {
        var psicologos = new List<ApplicationUser>();
